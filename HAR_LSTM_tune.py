"""
Bayesian hyperparameter tuning for the HAR-LSTM hybrid using Optuna
(TPE sampler + Median pruner).

HAR-LSTM = HAR-RV linear model (Corsi, 2009) + an LSTM residual learner. Only
the LSTM residual learner has hyperparameters; the HAR-RV part is fit by OLS on
the train window inside the dataset. This script tunes the LSTM residual learner
against the validation loss on the (standardised) HAR residual.

Run ONE study per horizon h (= --pred_len), since the best LSTM configuration is
expected to differ across h = 1, 5, 22. The look-back window is fixed at
...
Results are saved to 'HAR-LSTM results/<study_name>/' as a CSV and a
best_params.json. The Optuna study is persisted to
'HAR-LSTM results/<study_name>/study.db' (an SQLite RDB), so interrupted runs
resume automatically by re-running the same command. Override the location with
--output_dir.
...
The SQLite storage is fully compatible with optuna-dashboard:
    ...
    optuna-dashboard sqlite:///"HAR-LSTM results/<study_name>/study.db"
...
class TunableHARLSTM(Exp_HAR_LSTM):
    """Exp_HAR_LSTM with Optuna trial reporting and pruning hooks."""
...
def sample_hyperparameters(trial: optuna.Trial, base_cfg: argparse.Namespace) -> argparse.Namespace:
    ...
    # --- LSTM residual-learner architecture ---
    ...
    # internally, so RevIN is redundant. Keep it off.
    cfg.revin = 0
...
def objective(trial: optuna.Trial, base_cfg: argparse.Namespace) -> float:
    ...
    setting = f'harlstm_optuna_trial_{trial.number}'
    trial.set_user_attr('setting', setting)

    try:
        exp            = TunableHARLSTM(args)
...
def build_base_config(tune_args: argparse.Namespace) -> argparse.Namespace:
    ...
    # Sequence (pred_len IS the horizon h; the LSTM head always emits 1 value)
    ...
    # RevIN sub-settings (revin itself is fixed to 0 in sample_hyperparameters)
    cfg.affine        = 0
    cfg.subtract_last = 0
    cfg.aggregate_mean = False   # HAR target is already the h-day mean

    # Misc fields required by data_provider / model even though the LSTM ignores them
    cfg.embed_type     = 0
    cfg.n_heads        = 1
    cfg.d_layers       = 1
    cfg.distil         = True
...
def parse_tune_args():
    p = argparse.ArgumentParser(description='Optuna HPO for the HAR-LSTM hybrid')
    ...
    p.add_argument('--freq',      type=str, default='d',     help='Time feature frequency')
    ...
    p.add_argument('--output_dir',   type=str, default='./HAR-LSTM results/', help='Results output directory')
...
def main():
    ...
    study_name = tune_args.study_name or (
        f"harlstm_{tune_args.data}_h{tune_args.pred_len}"
    )
    ...
    study.set_user_attr('model', 'HAR_LSTM')
