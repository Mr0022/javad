"""
HAR-LSTM experiment: a hybrid of the HAR-RV linear model (Corsi, 2009) and an
LSTM residual learner.
...
2. The LSTM (models/LSTM.py) is trained to predict the (standardised) HAR
   residual from a look-back window of observed ln(RV). Early stopping uses
   the genuine validation split (2022-2023), because -- unlike pure HAR-RV --
   the LSTM has hyperparameters to tune.
...
per-horizon LSTM configuration of your choice.
"""
...
class Exp_HAR_LSTM(Exp_Basic):

    def _build_model(self):
        # The residual learner emits a single value per horizon, so the LSTM
    ...
    def test(self, setting, test=0):
        ...
        print('HAR-LSTM    mse:{:.4f}, mae:{:.4f}, rse:{:.4f}, qlike:{:.4f}'.format(
              h_mse, h_mae, h_rse, h_qlike))

        with open('result_har_lstm.txt', 'a') as f:
            f.write(setting + '\n')
            f.write('HAR-only  mse:{:.4f}, mae:{:.4f}, rse:{:.4f}, qlike:{:.4f}\n'.format(
                    l_mse, l_mae, l_rse, l_qlike))
            f.write('HAR-LSTM  mse:{:.4f}, mae:{:.4f}, rse:{:.4f}, qlike:{:.4f}\n\n'.format(
                    h_mse, h_mae, h_rse, h_qlike))
