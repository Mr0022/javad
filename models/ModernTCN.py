...
class ModernTCN(nn.Module):
    def __init__(self,patch_size,patch_stride, stem_ratio, downsample_ratio, ffn_ratio, num_blocks, large_size, small_size, dims, dw_dims,
        ...
        # news-event conditioning
        self.use_events = use_events
        self.event_past = event_past
        self.event_future = event_future
        self.event_fusion = event_fusion
        self.value_nvars = nvars
        # 'channel' fusion turns the past-event stream into an extra variable that
        # flows through the WHOLE backbone, so ModernTCN's cross-variable ConvFFN
        # mixes value<->events at every stage (not just a single stem injection).
        # The backbone is then built one variable wider and the event variable is
        ...
        if use_events:
            ...
            if event_past:
                ...
                # 'inject' adds this to the stage-0 map; 'channel' concatenates it
                # as a new variable.
                self.event_patch = nn.Sequential(
                    nn.Conv1d(event_dim, dims[0], kernel_size=patch_size, stride=patch_stride),
                    nn.BatchNorm1d(dims[0]))
            if event_future:
                # FiLM generator over the horizon's known event schedule;
                # zero-init => gamma=1, beta=0, so training starts as the
                # unconditioned model and the event path only activates
    ...
    def forward_feature(self, x, te=None, event_x=None):
        ...
        if self.use_events and self.event_past and event_x is not None:
            ev = self.event_embed(event_x)  # (B, L, event_dim)
            ev = ev.permute(0, 2, 1)        # (B, event_dim, L)
        else:
            ev = None
        ...
        for i in range(self.num_stage):
            ...
            if i == 0 and ev is not None:
                ...
                if self.event_as_channel:
                    # append the event stream as an extra variable; M grows to
                    # nvars+1 and is re-read at the top of the next iteration
                    x = torch.cat([x, ev_feat], dim=1)
                else:
                    # 'inject': add the (shared) event calendar to every variable
                    x = x + ev_feat
            x = self.stages[i](x)
        if self.event_as_channel:
            # drop the event variable so the head/RevIN see value variables only
